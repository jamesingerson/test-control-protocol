...
`TODO.md` also tracks a larger backlog not yet implemented: a colour-taxonomy revisit (splitting "user-declared variable" from "implicit global/system field", currently both rendered the same blue), hover/go-to-definition support for labels and variables, and a couple of not-yet-investigated candidates (`NORMALX`, a possible test-code reference).
